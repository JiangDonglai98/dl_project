'''
The data file should be placed in ./datasets 
The bert model file should be placed in ./tokenizer
transformer should be installed.
The pre trained Bert model can be download from https://github.com/valuesimplex/FinBERT.
Due to time limitation we do not save the weights for the classification layers. But you can train with the given data easily.
'''
...
# the following code is a general data manipulator, we only use part of it in this code.


bert_freeze = True # fix BERT parameters for non-classification layer
startDate = '2017-01-01' # start date of news
endDate = '2019-12-31' # end date of news
splitDate = '2019-06-01' # where to split and generate train set and test set

x_length = 512 # length that will be kept and given to BERT. ( <= 512)
num_labels = 2 # n-class classification 

# rank_list is used for n-class classification, the original label is continuous,
#  here we use the number in rank_list to discretelize labels
# rank_list = [-10, -1, -0.5, 0, 0.5, 1, 10]
rank_list = [-10,0,10]
if num_labels != 1:
    assert len(rank_list) - 1 == num_labels 
    # check if the rank_list is apropriate for number of class
...
class DataManipulator:
    ...
    def df_to_tensor(self, df: pd.DataFrame, label_list: list, split_time, is_train: bool, is_X: bool,
                     is_Bert: bool = False, bert_max_len: int = x_length, normal: bool = False) -> torch.Tensor:
            ...
            if not is_Bert:
                if not normal:
                    ...
                    result = normalize(torch.from_numpy(df.loc[df.index <= split_time, :].drop(
                        label_list, axis=1).values.astype(np.float32)), dim=1)
    ...
    def rank_minute_df_columns(self, target_col_name_list: list, rank_list=rank_list):
    ...
    def rank_df_column(self, target_col_name_list: list, rank_list=rank_list):
        self.label_num = len(rank_list) - 1
        for name in target_col_name_list:
    ...
    def scaling_col(self, scaler=StandardScaler, not_scal_list: list = ['text', 'rank', 'main_trade']):
        temp_scaler = scaler()
...
root = r''
token = '3f084a8c38805ecdecbbadc6cecdc86e2ee8fcd7810b33bd3818eb27'
...
if num_labels != 1:
    test_manipulator.LSTM_train_test_split('LSTM_without_sentiment', 20, start = startDate, end = endDate,
                                            split_date = splitDate, label_list=['rank_shift-1_ShangZheng_pct_chg'],
                                            news_col_name='sina_text')
    print(1, test_manipulator.start_date, test_manipulator.end_data, test_manipulator.split_date)

    test_manipulator.minute_LSTM_train_test_split('minute_LSTM_without_sentiment', 20, start = startDate, end = endDate,
                                            split_date = splitDate, label_list=['rank_shift-1_ShangZheng_30min_pct_chg'],
                                            news_col_name='sina_text')
    ...
    test_manipulator.Bert_train_test_split('Bert(7,9)', 1, start = startDate, end = endDate,
                                            split_date = splitDate, label_list=['rank_shift-1_ShangZheng_pct_chg'],
                                            news_col_name='sina_text')
    ...
    test_manipulator.Bert_train_test_split('Bert(15,17)', 1, start = startDate, end = endDate,
                                            split_date = splitDate, label_list=['rank_shift-1_ShangZheng_pct_chg'],
                                            news_col_name='sina_text')
    ...
    test_manipulator.minute_Bert_train_test_split('minute_Bert', 1, start = startDate, end = endDate,
                                            split_date = splitDate, label_list=['rank_shift-1_ShangZheng_30min_pct_chg'],
                                            news_col_name='sina_text')
    a = iter(test_manipulator.Bert_test_dict['Bert(15,17)1'])
else:
    test_manipulator.LSTM_train_test_split('LSTM_without_sentiment', 20, start=startDate, end=endDate,
                                            split_date=splitDate, label_list=['shift-1_ShangZheng_pct_chg'],
                                            news_col_name='sina_text')
    print(1, test_manipulator.start_date, test_manipulator.end_data, test_manipulator.split_date)

    test_manipulator.minute_LSTM_train_test_split('minute_LSTM_without_sentiment', 20, start=startDate, end=endDate,
                                                    split_date=splitDate,
                                                    label_list=['shift-1_ShangZheng_30min_pct_chg'],
                                                    news_col_name='sina_text')
    print(1, test_manipulator.minute_start_date, test_manipulator.minute_end_data,
            test_manipulator.minute_split_date)
    # Bert parameter selection separate
    test_manipulator.add_change_news('sina', (7, 9), columns_type, sina_news, time_col_name='create_time')
    test_manipulator.Bert_train_test_split('Bert(7,9)', 1, start=startDate, end=endDate,
                                            split_date=splitDate, label_list=['shift-1_ShangZheng_pct_chg'],
    ...
    print(2, test_manipulator.start_date, test_manipulator.end_data, test_manipulator.split_date)
    test_manipulator.add_change_news('sina', (11, 13), columns_type, sina_news, time_col_name='create_time')
    test_manipulator.Bert_train_test_split('Bert(11,13)', 1, label_list=['shift-1_ShangZheng_pct_chg'],
                                            news_col_name='sina_text')
    print(3, test_manipulator.start_date, test_manipulator.end_data, test_manipulator.split_date)
    test_manipulator.add_change_news('sina', (15, 17), columns_type, sina_news, time_col_name='create_time')
    test_manipulator.Bert_train_test_split('Bert(15,17)', 1, start=startDate, end=endDate,
...
batch_size = 64
Bert_dict_key = list(test_manipulator.Bert_train_dict.keys())[3]


train_dataloader = DataLoader(
            test_manipulator.Bert_train_dict[Bert_dict_key],  
            sampler = RandomSampler(test_manipulator.Bert_train_dict[Bert_dict_key]), 
            batch_size = batch_size 
        )


validation_dataloader = DataLoader(
            test_manipulator.Bert_test_dict[Bert_dict_key],  
            sampler = SequentialSampler(test_manipulator.Bert_test_dict[Bert_dict_key]),  
            batch_size = batch_size 
        )
# %%
...
for epoch_i in range(0, epochs):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        b_input_ids = batch[0][0].reshape(-1,lenght_of_sentence).to(device)
        b_input_mask = batch[0][1].reshape(-1,lenght_of_sentence).to(device)
        b_labels = batch[1].to(device)
        model.zero_grad()        
        returnL = model(b_input_ids, 
                                   token_type_ids=None, 
                                   attention_mask=b_input_mask,
                                   labels=b_labels.float() if num_labels == 1 else b_labels.long())
        loss = returnL[0]
        logits = returnL[1]
...
# a logger to record all settings and train log
import time
from pprint import pformat
with open(time.strftime("%Y_%m_%d_%H_%M", time.localtime()) + '.txt', 'w') as f:
    f.write('Bert_dict_key = {}\n'.format(Bert_dict_key))
    f.write('startDate = {}\n'.format(startDate))
    f.write('splitDate = {}\n'.format(splitDate))
    f.write('endDate = {}\n'.format(endDate))
    f.write('bert_freeze = {}\n'.format(bert_freeze))
    ...
    f.write('torch.cuda.memory_cached()/1024**2= {}\n'.format(torch.cuda.memory_reserved()/1024**2,'MB used in gpu'))

# the main train process end here.
# below is the code to use BERT as an fixed encoder to extract representations 
# the logic is even simpler.
# notice that this part need the first data manipulator in the beginnig of this code file
# %%

# these code load BERT model without add classification layer
...
for i in [3]:
    print("i :{}".format(i))
    Bert_dict_key = list(test_manipulator.Bert_train_dict.keys())[i]
 
    train_dataloader = DataLoader(
                test_manipulator.Bert_train_dict[Bert_dict_key],  
                shuffle = False, 
                # without shuffle 
                # since we only need the representation output by BERT without classification layer
                batch_size = batch_size 
            )

    # validation set is also encoded
    validation_dataloader = DataLoader(
                test_manipulator.Bert_test_dict[Bert_dict_key],  
                shuffle = False,
                batch_size = batch_size 
    ...
    oneTimeIntervalOutput = []

    for step, batch in enumerate(train_dataloader):
        print("before i {} step {} memory used{}".format(i,step,torch.cuda.memory_reserved()/1024**2))
        b_input_ids = batch[0][0].reshape(-1,lenght_of_sentence).to(device)
        ...
        oneTimeIntervalOutput.append((returnL['pooler_output'].cpu(),batch[1]))
        # extract the pooler output, which is of shape (n,768)

    #concatenate all epochs together for train
    x = torch.cat([i for i,j in oneTimeIntervalOutput])
    y = torch.cat([j for i,j in oneTimeIntervalOutput])
    allreturns[list(test_manipulator.Bert_train_dict.keys())[i]] = {}
    allreturns[list(test_manipulator.Bert_train_dict.keys())[i]]['train'] = (x,y)
    ...
    for step, batch in enumerate(validation_dataloader):
        ...
        print("after i {} step {} memory used{}".format(i,step,torch.cuda.memory_reserved()/1024**2))
    
    #concatenate all epochs together for test
    x = torch.cat([i for i,j in oneTimeIntervalOutput]) 
    y = torch.cat([j for i,j in oneTimeIntervalOutput])
    allreturns[list(test_manipulator.Bert_train_dict.keys())[i]]['test'] = (x,y)

# finally save all (encoded x, y) pairs 
for timeInterval, statedict in allreturns.items():
    for phase, xypair in statedict.items():
        x, y = xypair
        print(x.shape,y.shape)
        np.save('mini_{}_{}_{}_{}_x.npy'.format(timeInterval, phase, num_labels, x_length), x)
        np.save('mini_{}_{}_{}_{}_y.npy'.format(timeInterval, phase, num_labels, x_length), y)

'''
Reference:
Ryan, N., & McCormick, C. (2019, July 22). BERT fine-tuning tutorial WITH PYTORCH. Retrieved April 26, 2021, from https://mccormickml.com/2019/07/22/BERT-fine-tuning/#1-setup
'''